...
MODEL_DIR = "model"
MODEL_PATH = os.path.join(MODEL_DIR, "social_media_model.joblib")
...
def get_spark(app_name: str = "Social_Media_Predictions") -> SparkSession:
    """
...
def predict_with_model(model: RandomForestRegressor, df: pd.DataFrame) -> pd.DataFrame:
    ...
    df2 = df.copy()
    df2["Predicted_Engagement"] = preds
    return df2
